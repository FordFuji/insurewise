...
@section('content')
    <button class="btn btn-sm btn-primary m-r-5" type="button" onclick="window.location.href='{{url('backend/insurance_cancer/form')}}';" style="margin-bottom: 10px;">Add</button>
    ...
    <script>
        $(document).ready(function() {
			...
			var TableList = $('#data-table').dataTable({
				...
				dom: 'Bfrtip',
					"ajax": {
						"type": "POST",
						"url": "{{url('backend/insurance_cancer/server_processing')}}",
						"data": function (d) {},
						headers: {
					...
					"columns": [
						{"data" : "insurance_cancer_id"},
						{"data" : "insurance_company_name"},
						{"data" : "insurance_cancer_row"},
						{"data" : "action"},
					],
				});
        ...
        function clearData() {
            if(confirm('Confirm Delete') == true) {
                $.post('{{ url("ajaxClearInsuranceCancer") }}', { "_token": "{{ csrf_token() }}" }, function(data) {
